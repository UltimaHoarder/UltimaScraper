...
from helpers.main_helper import OptionsFormat, choose_option
...
async def start_datascraper(
    ...
    match site_name_lower:
        case "onlyfans":
            ...
            if not isinstance(api, OnlyFans.start):
                api = OnlyFans.start(max_threads=json_settings["max_threads"],config=Config(**json_config))
            ...
            auth_count = 0
            profile_options = OptionsFormat(api.auths, "profiles",auto_profile_choice)
            ...
            for auth in api.auths:
                ...
                setup, subscriptions = await module.account_setup(
                    auth, site_settings,identifiers, auth_count
                )
                if not setup:
                    if webhooks:
            ...
            subscription_options = OptionsFormat(subscription_array, "subscriptions",auto_model_choice)
